function varargout = T_rect_rod_z_y(z,y) 
...
gm = factor*J*L/(2.0*dsur); %power density [W/micron^2]
ga = (1-factor)*J*L/(L - 2.0*dsur); %power density [W/micron^2]
...
% Temperature of 0th order 
 ...
 for n=2*M-3:-2:1
      ...
      a0 = (2.0/L)*(ga*(L-2*dsur)+2.0*dsur*gm);
...
%  Temperature in kth order (see the references for details)
 for k = 1:1:1600
      ...
      for n=2*M-3:-2:1
       ...
       ak = (2.0*(ga-gm)/(k*pi))*sin(2.0*k*pi*(L/2.0-dsur)/L);

       A2K=ak/mu/(lambda(1)*(wA(2)+wB(2))*(wA(1)-wB(1))/(wA(1)+wB(1))-lambda(2)*(wA(2)-wB(2)));

      if (mod(nocurrentl,2) == 0)
         T=T+A2K*(wA(nocurrentl)*exp(mu*y)+wB(nocurrentl)*exp(-mu*y))*cos(mu*z);
      else
          T=T+((wA(2)+wB(2))/(wA(1)+wB(1)))*A2K*(wA(nocurrentl)*exp(mu*y)+wB(...
              nocurrentl)*exp(-mu*y))*cos(mu*z);
      end
  end
 varargout{1} = T;
